...
- Схема тренировок (`db/workout/`, `@Entity`-классы — с суффиксом `*DatabaseEntity`):
  ...
  - `is_archived` (у `workout` и `exercise`) — soft delete: сущность, на которую ссылаются сессии (и, для упражнения, программы), не удаляется физически, а прячется из списков/библиотеки (иначе повисли бы `workout_session.program_id` / `session_exercise.exercise_id`). Фильтруют `WHERE is_archived = 0` списки, поиск **и чтение программы по id** (`getWithExercises`): по удалённой программе нельзя ни начать тренировку, ни сохранить правку. Чтение упражнения по id (`ExerciseDao.getById`) и упражнения внутри графа программы — наоборот, архивные отдают: на них ссылаются живые программы и сессии. Запись графа флаг не сбрасывает (см. `saveWorkoutGraph`).
  ...
  - `WorkoutDao` (`abstract class`) — `getItems` (агрегирующий `@Query` с `LEFT JOIN workout_exercise` + `COUNT(...)` + `WHERE is_archived = 0`, возвращает `WorkoutItemWithExerciseCount`), `getWithExercises` (`@Transaction`, `WHERE is_archived = 0` — архивную программу не отдаёт, архивные упражнения внутри её графа отдаёт), гранулярные upsert/insert/delete, `isWorkoutArchived(id)`/`getArchivedExerciseIds(ids)` (служебные чтения флага для записи графа), `@Transaction saveWorkoutGraph(...)` (единый путь create/edit; для `workout`/`exercise` — `@Upsert`, а не `@Insert(REPLACE)`, иначе `INSERT OR REPLACE` снёс бы детей каскадом), `@Transaction updateSetTargets(workoutId, targets)` и `@Transaction deleteOrArchiveWorkout(id)` (если есть ссылающиеся сессии — `archiveWorkout`, иначе жёсткий `deleteWorkout`).
  - `saveWorkoutGraph(...)` переносит `is_archived` из текущих строк в апсертимые — в той же транзакции. Доменная модель про архив не знает и приносит `false` в каждой строке, поэтому запись «как принесли» вернула бы в списки удалённую программу и удалённое из библиотеки упражнение.
  - `updateSetTargets(...)` — путь прогрессии по итогам сессии: правит только `workout_set.count`/`weight` по позициям, строк `workout`/`exercise` не касается вовсе. Отдельно от `saveWorkoutGraph` намеренно — тот пересоздаёт связки и апсертит обе таблицы целиком. Позиции, которых в программе нет (её отредактировали после старта сессии), обновляют ноль строк — это не ошибка.
...
- Состояние приложения (`db/app/`):
  - `AppLaunchStateEntity` (`app_launch_state`) — singleton-строка (`id = AppLaunchStateEntity.SINGLETON_ROW_ID`, всегда `0`), `hasCompletedFirstLaunch: Boolean` + `hasCompletedOnboarding: Boolean` (`NOT NULL DEFAULT 0`). Переживает удаление любых доменных данных (например, если пользователь очистит библиотеку упражнений) — используется как независимый от содержимого других таблиц маркер «первый запуск уже прошёл» / «обучение уже прошли», а не эвристика вида «таблица X пуста». Значений по умолчанию у сущности нет намеренно: собрать её целиком и записать одним апсертом означало бы затереть флаг соседнего писателя.
  - `AppLaunchStateDao` (`abstract class`) — `get()` (singleton-строка или `null`, если ещё не создана), `insertIfAbsent(state)` (`@Insert(IGNORE)`), точечные `setFirstLaunchCompleted()`/`setOnboardingCompleted()` (`@Query UPDATE ... WHERE id = 0`) и две транзакции `@Transaction markFirstLaunchCompleted()`/`markOnboardingCompleted()` — «вставить строку при отсутствии → обновить свою колонку».
    Апсерта строки целиком **нет намеренно**: у флагов разные писатели (сид библиотеки на сплэше и выход из обучения), и второй, записав строку целиком, обнулил бы `has_completed_first_launch` — сид на следующем запуске засеял бы библиотеку повторно, продублировав все упражнения и программы. Инвариант держит `AppLaunchStateDaoTest` на настоящей SQLite.
- Миграции (`db/migration/`):
  ...
  - `MIGRATION_2_3` — флаг обучения: `ALTER TABLE app_launch_state ADD COLUMN has_completed_onboarding INTEGER NOT NULL DEFAULT 0`, затем бэкфилл `has_completed_onboarding = has_completed_first_launch`. Бэкфилл обязателен: без него в день обновления обучение вылезло бы всей установленной базе поверх реальных данных. На чистой установке строки нет, `UPDATE` меняет ноль строк — новый пользователь обучение увидит.
  ...
  - Тест — `Migration2To3Test`, по тому же образцу; из восьми таблиц v2 создаёт одну, которую миграция трогает. Дополнительно сверяет `pragma_table_info` мигрированной таблицы с таблицей, созданной по `createSql` из `schemas/…/3.json`: Room при открытии падает и на расхождении `DEFAULT`, а не только на отсутствующей колонке.
- Тесты SQL нетривиальных запросов — там же, в `androidHostTest` (`WorkoutSessionDaoProgramHistoryTest`, `WorkoutDaoArchiveTest`): предикаты, порядок и перенос `is_archived` проверяются на настоящей БД, потому что фейки DAO в core-модулях данных повторяют их вручную и доказать не могут.
...
Host-тесты модуля (`androidHostTest`) дополнительно тянут `libs.androidx.sqlite.framework`, `libs.robolectric` и `libs.junit` — ради тестов на настоящей SQLite: миграции (`Migration1To2Test`, `Migration2To3Test`, `LyteMigrationChainTest`) и SQL нетривиальных запросов DAO (`AppLaunchStateDaoTest` — независимость флагов запуска; `WorkoutSessionDaoProgramHistoryTest` — предикаты и порядок `getProgramSetHistory`, которые фейк DAO в `:core:core-session` повторяет вручную и потому доказать не может).
...
- **Текущая версия схемы — `3`.** История до первого релиза была схлопнута до `1.json`, дальше версии только растут. Любое изменение схемы обязано: (1) поднять `version`, (2) добавить `Migration`-объект в `db/migration/` и в `LYTE_MIGRATIONS` (`RoomBuilderDefaults.kt`), (3) закоммитить новый файл схемы (старые не трогаем), (4) добавить тест миграции по закоммиченным схемам — эталон `Migration1To2Test`. Деструктивный сброс в релизе запрещён.
  - Шаг (2) отдельно прикрыт `LyteMigrationChainTest`: тесты миграций вызывают свой `Migration` напрямую и потому не видят самую дорогую ошибку — миграция написана и оттестирована, но в `LYTE_MIGRATIONS` не попала. Сборка при этом зелёная, а установленная база не открывается вовсе. Тест поднимает БД настоящим Room-билдером, читает `user_version` и требует, чтобы цепочка вела от `1` до этой версии без пропусков.
...
- **Регистр в SQLite работает только для ASCII.** `LIKE`, `lower()` и коллация `NOCASE` не знают ничего про кириллицу: «жим» не найдёт «Жим лёжа», а строчное название уедет в конец `ORDER BY`. Поэтому под поиск/сортировку заводим служебную колонку с заранее нормализованным значением — как `exercise.name_normalized`, — а запрос нормализуем на стороне Kotlin. **Нормализация обязана быть одной функцией на оба конца сравнения** (в `core-workout` это `normalizedForSearch()`): колонка и запрос сравниваются через `LIKE`, и любое расхождение правил — это тихая деградация поиска. Сегодня правило — нижний регистр плюс неразрывный пробел (U+00A0), заменённый обычным: неразрывный подставляют клавиатуры, и без замены запрос с обычным пробелом не совпал бы с именем. Строки, которые пользователь вводит и которые попадают в `LIKE`, обязаны экранироваться (`%`, `_`, `\`) — в запросе для этого есть `ESCAPE '\'`.
