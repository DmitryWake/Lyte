...
- Контракты (`domain.repository`): `WorkoutRepository` (программы), `WorkoutExerciseRepository`
  ...
  - `WorkoutRepository.getWorkout(id)` — удалённую (архивную) программу не отдаёт: `null`. Вызывающий
    обязан показать это состояние, а не работать дальше с последним прочитанным графом — иначе по
    удалённой программе стартует тренировка с уже открытого превью.
  - `WorkoutRepository.updateWorkoutTargets(workoutEntity)` — узкая запись **только целей** подходов
    ...
    позициям; состав и порядок упражнений и число подходов берутся из БД и не меняются.
    Отдельно от `editWorkout` намеренно: тот пересобирает граф программы целиком, а прогрессии нужны
    только цели.
...
- Удаление программы/упражнения — **soft-delete** (архивирование), если на них ссылаются другие записи;
  подробности инвариантов — в `:core:core-db` README. Архивная программа невидима целиком (нет ни в
  списках, ни в `getWorkout`), архивное упражнение — только в библиотеке: программы и сессии, которые
  на него ссылаются, продолжают его показывать. Сохранение программы (`createWorkout`/`editWorkout`)
  флаг архива не сбрасывает ни ей самой, ни её упражнениям — его переносит DAO при записи графа.
- `WorkoutExerciseRepository.getExercises(query)` ищет по служебной колонке `exercise.name_normalized`
  через `LIKE`. И колонку (при записи, в маппере), и запрос нормализует **одна** функция
  `normalizedForSearch()`: нижний регистр плюс неразрывный пробел, заменённый обычным. Правило,
  разъехавшееся между двумя концами сравнения, ничего не ломает громко — упражнение просто перестаёт
  находиться тем запросом, который перекрывает эту позицию. Имя пользователя при этом не меняется:
  нормализация живёт только в служебной колонке.
