...
`CircleArrowUp`, `CircleArrowDown` (превысил/недобрал цель), `CircleMinus` (кружки-статусы подходов).

Словарь держит только то, что реально рисуется. `CircleX` удалён вместе с переписанным
`LyteTrackSetRow`: в v2 недобор до цели — это направление (стрелка вниз), а не провал, и крестик
рядом с подходом, который человек реально сделал, читается наказанием. `OverflowMenu` в дизайне v2
тоже не используется, но пока жив его call-site — кебаб-меню уходит вместе со списком программ
(RD-13).
...
| `component.session` | `LyteExerciseSetList` (композиция экрана трекинга — все подходы упражнения с закреплённой фокус-карточкой, см. ниже), `LyteTrackSetRow` (один подход: спокойная строка либо фокус-карточка, состояние — `LyteTrackSetState.Quiet`/`Focus`), `LyteSetDots`, `LyteSetOverview` (`currentIndex` — автопрокрутка к плашке текущего подхода), `LyteExerciseStrip` |
...
### Список подходов на экране трекинга

**`LyteTrackSetRow(number, state)`** рисует один подход в двух взаимоисключающих формах — их задаёт
`LyteTrackSetState`:

| Арм | Что это |
|---|---|
| `Quiet(tone, value, note)` | спокойная строка 36dp: индекс → иконка направления → заметка одной строкой с многоточием → значение справа |
| `Focus(setCount, reps, weight, references)` | приподнятая фокус-карточка: шапка «Подход N … из M», строки-ориентиры, степперы повторов и веса, слот `focusContent` |

Тон спокойной строки — общий `LyteProgressTone`, а не собственный словарь: один и тот же исход
обязан выглядеть одинаково и в треке, и в строке подхода, и в деталях сессии. Направление несёт
**иконка** — галочка, стрелка вверх, стрелка вниз, минус; **крестика нет ни в одном состоянии**.

Значение подхода — `LyteSetValue(reps, weight)`, где `weight = null` означает «свой вес» (строка
печатает «12 повт»). Ноль как признак отсутствия веса не используется: в домене вес тоже nullable, и
второе кодирование того же факта разъехалось бы с первым. У `Quiet` значение целиком nullable —
у пропущенного подхода его нет. У `Focus` повторы и вес лежат отдельными полями: их правят двумя
независимыми степперами, и `weight = null` там точно так же убирает секцию веса.

Ориентиры (`references`) — sealed `LyteTrackSetReference.Target`/`LastTime`: вызывающая сторона
решает, какие показать и в каком порядке, но **не как их назвать** — подписи принадлежат
дизайн-системе, потому что это тот же словарь, которым говорит спокойная строка будущего подхода
(«цель …»). Отдельными строками, а не одной склейкой «цель … · в прошлый раз …»: та переносилась по
словам.

Высота спокойной строки задана жёстко (36dp), а не паддингами: стили значений наследуют разный
`lineHeight` (24/20/16sp), и от паддингов строки разных тонов встали бы на разную высоту.

Обводка фокус-карточки — `Surface.border`, то есть **внутрь** границ. Наружная (или `outline`) была
бы срезана скроллером, когда карточка стоит у нижнего края списка; в вебе того же добивались
`inset box-shadow`.

**`LyteExerciseSetList(sets, onRepsChange, onWeightChange)`** — композиция всего экрана трекинга:
подходы упражнения по порядку, текущий — фокус-карточка, закреплённая у нижнего края области.

Фокус-карточка занимает ~200dp, спокойная строка — 36dp, поэтому после шести-семи подходов список
выше доступного места, и в обычном скроллере уезжает именно карточка — единственный элемент, которого
человек касается. В вебе это `position: sticky; bottom: 0`; в Compose sticky снизу нет, поэтому
перенесено **поведение**, а не приём: один скроллер, карточка — обычный элемент, а её низ доводится
до 104dp над низом области. Пока список короткий, карточку держит на якоре верхняя распорка
(`max(0, высота области − 104dp − высота головы списка)`) и прокрутка не нужна вовсе; дальше её
место занимает программная прокрутка — мгновенная на первом проходе и на `motion.durationMedium` с
`easingEmphasized` при смене подхода. Нижняя распорка ровно в 104dp гарантирует, что якорь достижим
и когда будущих подходов не осталось.

Верхняя распорка считается **в том же measure-проходе**, что и голова списка (`Modifier.layout`), а
не из состояния прошлого layout: иначе между закрытием подхода и рекомпозицией остаётся кадр, в
котором карточка стоит на строку ниже якоря и потом скачком возвращается. Индекс подхода и высота
головы публикуются одной парой, поэтому прокрутка запускается один раз, а не дважды подряд —
сначала по устаревшей высоте, потом по новой.
...
Горизонтальные поля (20dp) компонент держит сам: маска верхнего края рисуется в offscreen-слое,
который клипует строго по своим границам, и без полей срезал бы боковую тень фокус-карточки. Сама
маска — 20dp (`CompositingStrategy.Offscreen` + `BlendMode.DstIn`), выполненная строка проявляется со
сдвигом 8dp. Слот `footer` — под подпись в конце списка («Последний подход»): она обязана ехать в том
же скроллере, иначе отвяжется от конца.

```kotlin
LyteExerciseSetList(
    sets = listOf(
        LyteTrackSetState.Quiet(tone = LyteProgressTone.Met, value = LyteSetValue(reps = 10, weight = 60.0)),
        LyteTrackSetState.Focus(
            setCount = 3,
            reps = 10,
            weight = 62.5,
            references = listOf(LyteTrackSetReference.Target(value = "10×62.5 кг")),
        ),
        LyteTrackSetState.Quiet(tone = LyteProgressTone.Todo, value = LyteSetValue(reps = 10, weight = 62.5)),
    ),
