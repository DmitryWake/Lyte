...
- `com.nikolaevskii.lyte.core.design.format.formatWeight(weight: Double): String` — единый формат
  веса: целый — «60», дробный — «62,5» (запятая, округление до сотых). Одна реализация на все места,
  где вес показывается, **включая поле `LyteStepper`**: собственный формат у степпера означал бы
  «Цель 10 повт × 62,5 кг» над полем с «62.5», и компилятор такое не ловит.
- `com.nikolaevskii.lyte.core.design.format.lyteSetValueLabel(value: LyteSetValue, format: LyteSetValueFormat): String`
  (`@Composable`) — единый формат значения подхода. Форму выбирает **вызывающий**:
  `LyteSetValueFormat.Compact` — «10×62,5 кг» (спокойные строки списка, пилюли целей, плановые
  подходы в шторке: узкая строка держит 36dp, и семь таких обязаны поместиться рядом с
  фокус-карточкой); `LyteSetValueFormat.Expanded` — «10 повт × 62,5 кг» (ориентиры «Цель» и
  «В прошлый раз» в фокус-карточке, где строк две и место есть). Значения по умолчанию нет намеренно:
  выбор формы — решение места вызова, а не молчаливый дефолт.
  ...
  места нет.
- Между числом и его единицей — **неразрывный пробел** (U+00A0): «62,5 кг» не переносится так, чтобы
  «кг» уехало на следующую строку. Только на выводе; в хранимые данные он не попадает.
...
| `component.iconbutton` | `LyteIconButton` (размер = и круг, и зона касания; по умолчанию `LyteTheme.hitTarget.min`) |
...
Формы записи в двух местах карточки **разные, и это не рассинхрон**: спокойная строка пишет
компактно («10×62,5 кг»), ориентиры «Цель» и «В прошлый раз» — развёрнуто («10 повт × 62,5 кг»).
Строка держит 36dp и стоит в стопке из семи, ориентиров всего два; обе формы считает один
`lyteSetValueLabel`, поэтому разделителем и округлением разъехаться они не могут.
