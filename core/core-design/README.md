...
сводка завершённой сессии — это `Tones`, но карточка не запрещает и `Progress` («сколько подходов
позади»). Экранами `Progress` сейчас не используется — живёт только в превью.
...
- **Экраны 3.3 и 3.4 макета реализованы шторками, а не pushed-роутами.** В `LyteScreen.dc.html`
  выбор упражнения и редактор подходов — отдельные экраны с `TopBar`; в коде это `LyteBottomSheet`
  height=Full внутри редактора программы (решение 3 из `roadmap/design-v2/TASKS.md`). Визуально
