 ...
 *
 * [track] принимает режим целиком, а не список тонов: сводка завершённой сессии рисуется
 * `LyteProgressTrackMode.Tones` (исход каждого подхода высотой сегмента), но карточка не запрещает
 * и `Progress` — «сколько подходов позади» без разбора исходов. `null` — трека нет вообще.
 *
