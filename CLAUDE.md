...
Две команды:
- `/epic <описание>` — довести крупную задачу до роадмапа: разведка агентами → вопросы только по
  по-настоящему неоднозначному → разбивка на задачи размером в PR с полями «Зависимости» и
  «Затрагивает».
