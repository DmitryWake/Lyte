...
- Тесты живут в модулях, где есть логика: `:core:core-mvi`, `:core:core-db`, `:core:core-app`, `:core:core-workout`, `:core:core-session`, `:feature:{tracker,workout,history,splash,onboarding}:impl`. Гейт (гонять перед коммитом/релизом, macOS):
  - Host/JVM: `./gradlew :core:core-mvi:testAndroidHostTest :core:core-db:testAndroidHostTest :core:core-app:testAndroidHostTest :core:core-workout:testAndroidHostTest :core:core-session:testAndroidHostTest :feature:tracker:impl:testAndroidHostTest :feature:workout:impl:testAndroidHostTest :feature:history:impl:testAndroidHostTest :feature:splash:impl:testAndroidHostTest :feature:onboarding:impl:testAndroidHostTest`
...
- `:core:core-app` — data-слой состояния приложения: `AppLaunchStateRepository` (флаги «первый запуск пройден» и «обучение пройдено») + impl над `AppLaunchStateDao`, `coreAppModule()`, общий фейк `FakeAppLaunchStateRepository` в `testing`. Флаги независимы и пишутся точечно: общая запись строки затёрла бы чужой флаг и заставила бы сплэш засеять библиотеку повторно. Namespace `com.nikolaevskii.lyte.core.app`. Подробности — `core/core-app/README.md`.
...
- `:feature:onboarding:{api,impl}` — обучение: приветствие и тур. **Один маршрут на все кадры**, шаг — поле `UiState`: пять маршрутов дали бы пять ViewModel и запись флага «обучение пройдено» в пяти терминальных точках. Маршрут верхнего уровня, брат `SplashRoute`, поэтому плавающий док скрывается сам и `App()` не правится. Развилка «приветствие или трекер» живёт в `SplashViewModel`, а не в самом обучении.
...
- **Корень `NavHost` — `SplashRoute`** (`splashGraph()`), не `BottomNavGraph`: приложение стартует со сплэша, и после инициализации VM сплэша уходит либо на `OnboardingRoute` (флаг «обучение пройдено» не выставлен), либо сразу на `TrackerLandingRoute` — в обоих случаях с `LyteNavOptions(popUpTo = SplashRoute, popUpToInclusive = true)`. Развилка живёт в `SplashViewModel`: тур не сделан корнем графа с самороспуском, чтобы решение «с чего стартует приложение» осталось в одном месте.
- **`OnboardingRoute` — маршрут верхнего уровня**, брат `SplashRoute`, а не вкладка. Плавающий док прячется сам: `isTopLevelSelected` не находит вкладку, и `showBottomBar` становится `false` — `App()` для обучения не правился.
...
- Драйвер — `BundledSQLiteDriver` (androidx.sqlite), `setQueryCoroutineContext(Dispatchers.IO)`, `addMigrations(*LYTE_MIGRATIONS)` — см. `RoomBuilderDefaults.applyLyteDefaults()`. Текущая версия схемы — **3** (v1→v2 добавила маркер: `accent`/`glyph` у `exercise` и `workout`, `program_accent`/`program_glyph` — снапшот в `workout_session`; v2→v3 — `has_completed_onboarding` у `app_launch_state` с бэкфиллом из `has_completed_first_launch`); деструктивного `fallbackToDestructiveMigration` нет — только `Migration`-объекты в `db/migration/` (подробности — `core-db/README.md`).
