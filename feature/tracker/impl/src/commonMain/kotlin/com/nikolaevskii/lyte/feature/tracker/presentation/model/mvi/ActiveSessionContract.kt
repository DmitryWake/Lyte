...
sealed interface ActiveSessionIntent : UiIntent {
    ...
    /**
     * Подтверждение досрочного завершения: оставшиеся подходы — пропущенные, дальше тот же путь,
     * что и у обычного финиша, — детали завершённой сессии во вкладке «История». Лендинг трекера в
     * этом маршруте промежуточный: им заменяется активная сессия в стеке вкладки, чтобы возврат на
     * трекер не показывал мёртвый экран. Последнее, что видит пользователь, — результат сессии.
     */
