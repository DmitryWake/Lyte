...
/**
 * Скриншоты всех `@Preview` фичи «Трекер». Новое превью попадает под контроль автоматически —
 * править этот файл не нужно.
 */
@RunWith(ParameterizedRobolectricTestRunner::class)
class TrackerPreviewScreenshotTest(case: LytePreviewCase) : LytePreviewScreenshotTest(case) {

    companion object {
        @JvmStatic
        @ParameterizedRobolectricTestRunner.Parameters(name = "{0}")
        fun cases(): List<LytePreviewCase> =
            lytePreviewCases(previewPackage = "com.nikolaevskii.lyte.feature.tracker")
    }
}
