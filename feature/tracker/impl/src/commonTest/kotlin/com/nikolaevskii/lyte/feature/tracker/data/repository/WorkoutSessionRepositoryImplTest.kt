...
class WorkoutSessionRepositoryImplTest {
    ...
    @Test
    fun getLastSessionDatesReturnsLatestPerProgram() = runTest {
        val clock = MutableClock(START_MILLIS)
        val repository = repository(clock)
        ...
        val dates = repository.getLastSessionDates()

        assertEquals(1, dates.size)
        assertEquals("prog-1", dates.single().programId)
        assertEquals(Instant.fromEpochMilliseconds(LATER_MILLIS), dates.single().finishedAt)
    }
